...
* Blogs :blog:
:PROPERTIES:
:ID:       e88e1388-bd02-4146-9014-b3f693246605
:END:
...
To remove all =.elc=, run (not recommend.)
#+BEGIN_SRC
...
:PROPERTIES:
:ID:       8b381186-77ae-416c-ac78-70f5364b7711
:END:
...
**** edebug mode
:PROPERTIES:
:ID:       be24fe6d-24f3-4a06-b163-a9488daa2703
:END:
- ref
  - tutorial
...
:PROPERTIES:
:ID:       babc26d9-a9dd-42ea-953b-18c207005031
:END:
*** print debugging in elisp
http://xahlee.info/emacs/emacs/elisp_printing.html

*** check if elisi function is defined
...
*** regression test
*** use trace.el to  trace the execution of fucntion involved in the problem.
the packages provide functions
- ~trace-function-foreground~ and ~trace-function-background~ for tracing function calls
- ~trace-values~ for adding values of select vairables to the trace.
#+BEGIN_SRC emacs-lisp
(defun hi ()
  "hi")

(trace-function-foreground 'hi)
(hi)
...
(fact 5)
(trace-function-foreground 'fact)
#+END_SRC
*** debug-watch
